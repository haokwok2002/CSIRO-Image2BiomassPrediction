...
class DualStreamDatasetGPU(Dataset):
    def __init__(self, df, image_dir, config, device, transform=None):
        """
        一次性将所有图像加载进 GPU。
        df: 含 image_path 与 target 列的 DataFrame
        image_dir: 图像根目录
        config: 配置字典，需含 target_cols
        device: GPU 设备
        ...
        print(f"🚀 正在将 {len(df)} 张图片加载到 GPU (size={2000}x{1000}) ...")

        for i, row in tqdm(enumerate(df.itertuples()), total=len(df)):
            # 加载并转为RGB
            img_name = Path(row.image_path).name  # 提取文件名部分
            ...
            # # FP16 存储，减少显存一半
            # img_left = img_left.half().to(device, non_blocking=True)
            # img_right = img_right.half().to(device, non_blocking=True)
            # ✅ 直接搬到GPU
        ...
        print(f"✅ 加载完成，当前显存占用: {torch.cuda.memory_allocated(device)/1e9:.2f} GB")
    ...
    def __getitem__(self, idx):
        if self.target_cols is not None:
            return self.img_left_all[idx], self.img_right_all[idx], self.targets_all[idx]
        else:
...
def train_with_groupkfold(  
    ...
    for fold, (train_idx, val_idx) in enumerate(gkf.split(df, groups=groups)):
        ...
        train_dataset = DualStreamDatasetGPU(train_df, DIRS["train"], config, device,
                                            transform=get_train_transforms(config["img_size"]))
        valid_dataset = DualStreamDatasetGPU(valid_df, DIRS["train"], config, device,
...
config = {
    "py_file_name"                   : f"{Path(__file__).name}",
    # ⚙️ 基础训练参数
    "epochs"                         : 300,
    "freeze_ratio"                   : 0.6,
    "num_workers"                    : 0,
    ...
    "img_size"                       : 1000,
    "backbone_name"                  : "convnext_tiny",
    "cv_stability_stop_threshold"    : 0.015,
