...
def select_free_gpu(threshold_mem_MB=500, threshold_util=5):
    """自动选择一块空闲 GPU（按显存和利用率阈值）"""
    gpus = get_gpu_info()
    free_gpus = [
        g for g in gpus
    ...
    if not free_gpus:
        # 如果没有完全空闲的，就选显存最少占用的那块
        gpus.sort(key=lambda x: x["mem_used_MB"])
        selected = gpus[0]
        reason = "（未找到空闲 GPU，选显存占用最少的）"
    else:
    ...
    print(tabulate(
        [[g["index"], g["name"], f"{g['mem_used_MB']}/{g['mem_total_MB']} MB",
         ...
         for g in gpus],
        headers=["GPU", "Name", "Memory", "Util", "Temp", "Power"], tablefmt="grid"
    ))

    print(f"\n✅ 已选择 GPU {selected['index']} {reason}\n")

    # 设置环境变量
    os.environ["CUDA_VISIBLE_DEVICES"] = str(selected["index"])
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"当前使用设备: {device} (GPU {selected['index']})")

    return selected["index"], device
...
class MyDualStreamModel(nn.Module):
    ...
    def forward(self, img_left, img_right):
        ...
        preds_full = torch.cat([G, C, D, GDM, Total], dim=1)
    ...
    def compute_loss(self, preds, targets):
        ...
        # 解包预测与真值（顺序与 forward 保持一致）
        G, C, D, GDM, Total = [preds[:, i] for i in range(5)]
        G_t, C_t, D_t, GDM_t, Total_t = [targets[:, i] for i in range(5)]

        # ✅ 分别计算五个目标的 loss
        loss_G      = l1(G,      G_t)
        loss_C      = l1(C,      C_t)
        loss_D      = l1(D,      D_t)
        loss_GDM    = l1(GDM,    GDM_t)
        loss_Total  = l1(Total,  Total_t)
...
# 数据集加载定义  训练GPU 预测CPU
class DualStreamDatasetGPU(Dataset):
    # GPU版本：在初始化时一次性将所有图像加载到 GPU。
    def __init__(self, df, image_dir, config, device, transform=None):

        self.df = df.reset_index(drop=True)
        self.image_dir = Path(image_dir)
        self.device = device
        self.transform = transform
        self.target_cols = config.get("target_cols", None)

        self.img_left_all = []
        self.img_right_all = []
        self.targets_all = []

        print(f"🚀 正在将 {len(df)} 张图片加载到 GPU ...")
        ...
        for i, row in tqdm(enumerate(df.itertuples()), total=len(df)):
            ...
            self.img_left_all.append(img_left)
            self.img_right_all.append(img_right)


            # === 4️⃣ 返回结果 ===
            if self.target_cols is not None:
                t = torch.tensor(
                    [getattr(row, col) for col in self.target_cols],
                    dtype=torch.float32,
                    device=device
                )
                self.targets_all.append(t)


        mem = torch.cuda.memory_allocated(device) / 1e9
        print(f"✅ 所有图像已加载到 GPU，占用约 {mem:.2f} GB 显存。")
    ...
    def __getitem__(self, idx):
        if self.target_cols is not None:
            return (
                self.img_left_all[idx],
                self.img_right_all[idx],
                self.targets_all[idx],
            )
        else:
            return self.img_left_all[idx], self.img_right_all[idx]
...
def train_one_epoch(model, dataloader, optimizer, device, scaler):
    ...
    for step, (img_left, img_right, targets) in enumerate(dataloader):
        ...
        #     f"data load: {data_load_time*1000:6.1f} ms | "
        #     f"to(device): {(t1-t0)*1000:6.1f} ms | "
        #     f"forward+loss: {(t2-t1)*1000:6.1f} ms | "
        #     f"backward+opt: {(t3-t2)*1000:6.1f} ms | "
        #     f"total: {(t3-prev_end)*1000:8.1f} ms"
        # )
...
def validate_one_epoch(model, dataloader, device):
    ...
    with torch.no_grad():
        for step, (img_left, img_right, targets) in enumerate(dataloader):
            ...
            # if step  == 0  or step  == 1:
            # print(
            #     f"[VAL__] Step {step:4d} | "
            #     f"data load: {data_load_time*1000:6.1f} ms | "
            #     f"to(device): {(t1 - t0)*1000:6.1f} ms | "
            #     f"forward+loss: {(t2 - t1)*1000:6.1f} ms | "
            #     f"backward+opt: {(0)*1000:6.1f} ms | "
            #     f"total: {(t2 - prev_end)*1000:8.1f} ms"
            # )
...
def train_with_groupkfold(  
    ...
    for fold, (train_idx, val_idx) in enumerate(gkf.split(df, groups=groups)):
        ...
        train_dataset = DualStreamDatasetGPU(train_df, DIRS["dir"], config, device,
                                            transform=get_train_transforms(config["img_size"]))
        valid_dataset = DualStreamDatasetGPU(valid_df, DIRS["dir"], config, device,
                                            transform=get_valid_transforms(config["img_size"]))
        train_loader = DataLoader(
            train_dataset,
            batch_size=config["batch_size"],
            shuffle=True,
            num_workers=0,       # ❌ 不需要 worker
            pin_memory=False,    # ❌ 不需要 pin_memory
        )
...
config = {
    ...
    "img_size"                       : 768,
    "backbone_name"                  : "convnext_tiny",
    "cv_stability_stop_threshold"    : 0.001,
...
if __name__ == "__main__" and isTRAIN: 
    ...
    if socket.gethostname() == "user-PowerEdge-XE9680":
        ...
        print(f"✅ 源代码 {source_path.name} 已复制到: {target_path}")

        print(f"\n\n\n")

        # 读取训练数据
        df_train = load_and_prepare_train_df()
        if DEBUG: df_train = df_train.iloc[:30].copy()
...
def predict_ensemble_df(df_test, transform, model, model_target_cols, model_dir, device, batch_size=32, img_size=768):
    ...
    for fold, model_path in enumerate(model_paths):
        ...
        if socket.gethostname() == "user-PowerEdge-XE9680" and fold > 6:
            print("✅ ✅ ✅ ✅ ✅ 当前是训练阶段的预测演习✅ ✅ ✅提前退出 ✅ ✅ ✅ ✅ v ")
            break
