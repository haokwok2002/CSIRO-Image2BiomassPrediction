...
class MySingleStreamModel(nn.Module):
    def __init__(self, 
        ...
        self.weights_compute_loss = torch.tensor([0.1, 0.1, 0.1, 0.2, 0.5], device = device)
        
        b = config["batch_size"]
        w_list = [
            torch.full(
                (b,),                       # 对应 targets[:, i] 的长度
                self.weights_compute_loss[i].item(),
                dtype=torch.float32
            )
            for i in range(5)
        ]
        self.w_flat_compute_loss = torch.cat(w_list, dim=0).to(device)   # 形状：[b * 5]

        print(self.weights_compute_loss)
        print(self.w_flat_compute_loss)
    ...
    def compute_loss(self, preds, targets):
        ...
        # Start measuring time for the entire function
        start_time = time.time()
        
        print(get_current_time_with_ms(),"   compute_loss_start-----")

        # Flatten preds and targets
        preds = preds.view(-1, 5)
        targets = targets.view(-1, 5)
        
        print(get_current_time_with_ms(),"   Flatten")
        
        
        # Predefined weights for each target
        weights = self.weights_compute_loss
        print(get_current_time_with_ms(),"   weights")

        # Flatten targets and preds
        y_true_flat = targets.view(-1)
        y_pred_flat = preds.view(-1)
        print(get_current_time_with_ms(),"   Flatten targets and preds")

        # Vectorized weight assignment

        if preds.shape[0] == config["batch_size"]:
            w_flat = self.w_flat_compute_loss
        else:
            w_flat = torch.cat([
                torch.full_like(targets[:, i], weights[i], device=preds.device)
                for i in range(5)
        ...
        print(get_current_time_with_ms(),"   Flatten w_flat")

        # Compute mean of targets
        y_mean = torch.sum(w_flat * y_true_flat) / torch.sum(w_flat)
        print(get_current_time_with_ms(),"   Compute mean of targets")

        # Compute ss_res (residual sum of squares) and ss_tot (total sum of squares)
        ss_res = torch.sum(w_flat * (y_true_flat - y_pred_flat) ** 2)
        ss_tot = torch.sum(w_flat * (y_true_flat - y_mean) ** 2)
        print(get_current_time_with_ms(),"   Compute ss_res")

        # Calculate and return the loss
        loss = ss_res / ss_tot
        print(get_current_time_with_ms(),"   Compute loss")
...
class SingleStreamDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.pre_transform:
            image = add_shadow(image=image, shadow_area_ratio=0.5, num_shadows_range=(5, 10), blur_radius=10, p=0.5)
            image = add_glare(image=image, glare_area_range=(1000, 2500), num_glares_range=(10, 30), blur_radius=1, p=0.5)
...
def add_shadow(image, shadow_area_ratio=0.5, num_shadows_range=(5, 10), blur_radius=10, p=0.5, isshow=False):
    """
    在图像中添加随机的影子块。
...
def add_glare(image, glare_area_range=(1000, 2500), num_glares_range=(10, 30), blur_radius=1, p=0.5, isshow=False):
    """
    在图像中添加纯白色太阳直射反光光斑。
...
def train_one_epoch(model, dataloader, optimizer, device, scaler, config):
    ...
    for step, (images, targets) in enumerate(dataloader):
        print(get_current_time_with_ms(),"   enter train_one_epoch----------------------")
        t_load = time.time()  
        data_load_time = t_load - prev_end
        print(get_current_time_with_ms(),"   data_load_time")
        t0 = time.time()
        images = images.to(device, non_blocking=True).contiguous(memory_format=torch.channels_last)
        targets = targets.to(device, non_blocking=True)
        t1 = time.time()
        print(get_current_time_with_ms(),"   to(device)")
        ...
        if use_mixup:
            ...
            with torch.amp.autocast(device_type='cuda'):
                preds = model(images)
                print(get_current_time_with_ms(),"   forward")
                t_test_0 = time.time()
                
                print(get_current_time_with_ms(),"   enter model.compute_loss   train")
                loss = model.compute_loss(preds, targets)
                print(get_current_time_with_ms(),"   outer model.compute_loss   train")
                
        t2 = time.time()
        print(get_current_time_with_ms(),"   loss over")
        ...
        scaler.update()
        t3 = time.time()
        print(get_current_time_with_ms(),"   end train_one_epoch")
        ...
        print(
            ...
            f"forward: {(t_test_0 - t1)*1000:7.1f} ms | "
            f"loss: {(t2 - t_test_0)*1000:7.1f} ms | "
            f"backward+opt: {(t3 - t2)*1000:7.1f} ms | "
...
def validate_one_epoch(model, dataloader, device):
    ...
    # with torch.no_grad():
    #     for step, (images, targets) in enumerate(dataloader):
    #         print(get_current_time_with_ms(),"   enter validate_one_epoch----------------------")
    #         t_load = time.time()
    #         data_load_time = t_load - prev_end
    #         print(get_current_time_with_ms(),"   data_load_time")
    ...
    #         t_test_0 = time.time()
    #         print(get_current_time_with_ms(),"   enter model.compute_loss   val")
    #         val_loss = model.compute_loss(preds, targets)
    #         print(get_current_time_with_ms(),"   outer model.compute_loss   val")
    #         t2 = time.time()
    #         val_loss = val_loss.item()
    #         print(get_current_time_with_ms(),"   val_loss.item()")
    ...
    with torch.no_grad():
        for step, (images, targets) in enumerate(dataloader):
            ...
            images = images.to(device, non_blocking=True).contiguous(memory_format=torch.channels_last)
            targets = targets.to(device, non_blocking=True)
            torch.cuda.synchronize()   # 同步，让拷贝时间真实反映
            t1 = time.time()
            print(get_current_time_with_ms(), f"   to(device) cost: {(t1 - t0)*1000:.2f} ms")
            ...
            t3 = time.time()
            print(get_current_time_with_ms(), f"   forward cost: {(t3 - t2)*1000:.2f} ms")
            ...
            t5 = time.time()
            print(get_current_time_with_ms(),"   outer model.compute_loss   val",
                f"   loss compute cost: {(t5 - t4)*1000:.2f} ms")

            # item() —— 此时已经同步过了，这里应该非常快
            t6 = time.time()
            val_loss = val_loss.item()
            t7 = time.time()
            print(get_current_time_with_ms(),"   val_loss.item()",
                f"   item cost: {(t7 - t6)*1000:.4f} ms")
            ...
            print(get_current_time_with_ms(),"   loss append")
...
def train_one_fold(fold, df, groups, save_dir, config, device):
    ...
    for epoch in range(config["epochs"]):
        ...
        if epoch >= min_epoch:
            should_save_cv = len(top_cv_models) < 3 or cv > min(top_cv_models, key=lambda x: x[0])[0]
            if should_save_cv:
                cv_model_path = save_dir / f"model_cv_epoch{epoch+1}_fold{fold+1}.pt"
                torch.save(model.state_dict(), cv_model_path)
                
                top_cv_models.append((cv, epoch + 1, str(cv_model_path)))
                top_cv_models.sort(key=lambda x: x[0], reverse=True)
                if len(top_cv_models) > 3:
                    removed_cv = top_cv_models.pop()
                    try:
                        os.remove(removed_cv[2])
                        print(f"🗑️ 删除CV模型: {os.path.basename(removed_cv[2])}")
                    except:
                        pass
                print(f"🌟 更新CV   Top{len(top_cv_models)}模型！Fold {fold+1} | Epoch {epoch+1} | CV  ={cv:.4f}")

        should_save_loss = len(top_loss_models) < 3 or val_loss < max(top_loss_models, key=lambda x: x[0])[0]
        if should_save_loss:
            loss_model_path = save_dir / f"model_loss_epoch{epoch+1}_fold{fold+1}.pt"
            torch.save(model.state_dict(), loss_model_path)
            top_loss_models.append((val_loss, epoch + 1, str(loss_model_path)))
            top_loss_models.sort(key=lambda x: x[0])
            if len(top_loss_models) > 3:
                removed_loss = top_loss_models.pop()
                try:
                    os.remove(removed_loss[2])
                    print(f"🗑️ 删除Loss模型: {os.path.basename(removed_loss[2])}")
                except:
                    pass
            
            print(f"🌟 更新Loss Top{len(top_loss_models)}模型！Fold {fold+1} | Epoch {epoch+1} | Loss={val_loss:.4f}")
