...
class MyAugFeats(nn.Module):
    ...
    def forward(self, img):
        ...
        Pre_aux_feats = self.softplus(self.head_aux_feats(feat))
        preds = torch.cat([Pre_aux_feats], dim=1)  # [B, 1]
        return preds
...
class MyAugFeatsDataset(Dataset):
...
def get_fold_loaders(df, fold, config, device):
    ...
    train_dataset = MyAugFeatsDataset(
        train_df,
        DIRS["dir"], 
        config, 
