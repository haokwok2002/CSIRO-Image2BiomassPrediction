...
class CrossAttention(nn.Module):
    ...
    def forward(self, left, right):
        ...
        out, _ = self.attn(left, right, right)   # left attends to right
        out = self.norm(out + left)  # 残差 + 归一化
...
class MyDualStreamModel(nn.Module):
    def __init__(self, 
        ...
        self.cross_attn = CrossAttention(dim = in_dim)
    ...
    def forward(self, img_left, img_right):
        ...
        attn_left  = self.cross_attn(feat_left, feat_right)
        attn_right = self.cross_attn(feat_right, feat_left)

        fused = torch.cat([attn_left, attn_right], dim=1)
