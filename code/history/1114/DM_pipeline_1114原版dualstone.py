...
class MyDualStreamModel(nn.Module):
    def __init__(self, 
        ...
        if socket.gethostname() == "user-PowerEdge-XE9680":
            ...
            self.backbone_left = timm.create_model(backbone_name, pretrained=False, num_classes=0)
            self.backbone_right = timm.create_model(backbone_name, pretrained=False, num_classes=0)
            try:
                state_dict = torch.load(model_path, map_location='cpu', weights_only=True)
            except TypeError:
                state_dict = torch.load(model_path, map_location='cpu')
            self.backbone_left.load_state_dict(state_dict, strict=False)
            self.backbone_right.load_state_dict(state_dict, strict=False)
        else:
            self.backbone_left = timm.create_model(backbone_name, pretrained=pretrained, num_classes=0)
            self.backbone_right = timm.create_model(backbone_name, pretrained=pretrained, num_classes=0)

        in_dim = self.backbone_left.num_features

        # 2️⃣ Freeze partial backbone parameters
        params = list(self.backbone_left.parameters())
        freeze_until = int(len(params) * config["freeze_ratio"])
        for i, p in enumerate(params):
            p.requires_grad = i >= freeze_until     # freeze front part, train later part

        params = list(self.backbone_right.parameters())
        freeze_until = int(len(params) * config["freeze_ratio"])
        for i, p in enumerate(params):
            p.requires_grad = i >= freeze_until     # freeze front part, train later part
    ...
    def forward(self, img_left, img_right):
        # Extract features
        feat_left  = self.backbone_left(img_left)
        feat_right = self.backbone_right(img_right)
