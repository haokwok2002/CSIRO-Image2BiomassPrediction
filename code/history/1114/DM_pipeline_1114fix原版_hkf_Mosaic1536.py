...
class DualStreamDataset(Dataset):
    def __init__(self, df, image_dir, config, mode):
        ...
        # prepare transforms
        self.train_single_aug = get_train_single_aug()
        self.final_aug = get_final_aug(config["img_size"])
        self.tta_aug = get_tta_aug()
        self.Rotate90_aug = get_RandomRotate90()
    ...
    def load_image(self, idx):
        ...
        y = torch.tensor(row[self.target_cols].astype(float).values, dtype=torch.float32)
        return img, y
    ...
    def __getitem__(self, idx):
        # load first image
        imgA, yA = self.load_image(idx)

        if self.mode == "Train":
            # pick random second image
            idx2 = np.random.randint(0, len(self.df))

            imgB, yB = self.load_image(idx2)

            # single image augmentation
            imgA = self.train_single_aug(image=imgA)["image"]
            imgB = self.train_single_aug(image=imgB)["image"]

        else:
            # Val or TTA: use same image twice
            imgB, yB = imgA, yA

            # TTA transformations
            if self.mode in ["base", "hflip", "vflip"]:
                imgA = self.tta_aug[self.mode](image=imgA)["image"]
                imgB = self.tta_aug[self.mode](image=imgB)["image"]

        # concatenate
        img = np.concatenate([imgA, imgB], axis=0)

        if self.mode == "Train":
            img = self.Rotate90_aug(image=img)["image"]

        # final augment (resize + norm + tensor)
        img = self.final_aug(image=img)["image"]

        # combine labels (train uses two different images; val uses same image twice)
        y = yA + yB
...
# Albumentations
def get_train_single_aug():
    return A.Compose([
        A.HorizontalFlip(p=0.5),
        A.VerticalFlip(p=0.5),
        A.ColorJitter(p=0.3),
    ])
def get_RandomRotate90():
    return A.Compose([
        A.RandomRotate90(p=0.5)
    ])

def get_final_aug(size):
    return A.Compose([
        A.Resize(size, size),
...
def add_hkf_folds(df: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        # ==== 打印 stratify 分布 ====
        print("\n===== Stratify Key 分布统计 =====")
        count_series = df['stratify_key'].value_counts().sort_index()
        print(count_series)
        total_count = count_series.sum()

        print(f"\n总共有 {count_series.shape[0]} 个 stratify 类别，共计{total_count}\n")
...
def train_one_epoch(model, dataloader, optimizer, device, scaler):
    ...
    for step, (img, targets) in enumerate(dataloader):
...
def get_fold_loaders(df, fold, config, device):
    ...
    train_dataset = DualStreamDataset(train_df, DIRS["dir"], config, mode = "Train")
    valid_dataset = DualStreamDataset(valid_df, DIRS["dir"], config, mode = "Val")
...
def predict_ensemble(df_test, aug_name, model_dir, device, config):
...
def generate_submission(df_pred_final):
    ...
    df_submit["target"] = df_submit["target"] / 2
    print("✅✅✅✅✅ final data /2 ✅✅✅✅✅")
...
def run_tta_prediction(df_test, model_dir, device, config):
    ...
    for aug_name, transform in tta_transforms.items():
        print(f"\n🚀 TTA mode: {aug_name}")
        df_pred = predict_ensemble(df_test, aug_name, model_dir, device, config)
        all_preds.append(df_pred[config["target_cols"]].values)

        # ✅ Show intermediate results
        print(f"Preview of predictions for TTA mode [{aug_name}]:")
        print(df_pred.head())
