...
class MySingleStreamModel(nn.Module):
    def __init__(self, 
        ...
        # 3️⃣ 微调策略：混合策略 (LoRA + Partial Unfreeze)
        if config.get("use_lora", False):
            print(f"🚀 [Hybrid] Enabling LoRA + Unfreezing last {1-config.get('freeze_ratio', 0.8):.1%} layers...")
            
            # 先应用 LoRA (这会自动冻结 backbone 所有原始参数)
            ...
            # 手动解冻后部参数
            # 注意：peft 包装后，原始 backbone 在 self.backbone.base_model.model 中
            params = list(self.backbone.base_model.model.parameters())
            freeze_until = int(len(params) * config.get("freeze_ratio", 0.8))
            
            for i, p in enumerate(params):
                if i >= freeze_until:
                    p.requires_grad = True # 强制解冻
        else:
            # 纯冻结逻辑保持不变
...
def predict_ensemble(df_test, pre_transform, transform, model_dir, device, config):
    ...
    for fold, model_path in enumerate(model_paths):
        print(f"🔍 [{fold+1}/{len(model_paths)}] 推理中: {model_path.name}")
