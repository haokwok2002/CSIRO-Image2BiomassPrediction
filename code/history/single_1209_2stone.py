...
class MySingleStreamModel(nn.Module):
    def __init__(self, 
        ...
        if socket.gethostname() == "user-PowerEdge-XE9680":
            ...
            self.backbone1 = timm.create_model(backbone_name, pretrained=False, num_classes=0)
            try:
                state_dict1 = torch.load(model_path, map_location='cpu', weights_only=True)
            except TypeError:
                state_dict1 = torch.load(model_path, map_location='cpu')
            self.backbone1.load_state_dict(state_dict1, strict=False)
        else:
            self.backbone1 = timm.create_model(backbone_name, pretrained=pretrained, num_classes=0)

        params = list(self.backbone1.parameters())
        ...
        backbone_name = config["backbone_name2"]
        print("Current backbone:", backbone_name)
        if socket.gethostname() == "user-PowerEdge-XE9680":
            model_path = f"/data4/huangweigang/gh/timm_model/{backbone_name}.pth"
            self.backbone2 = timm.create_model(backbone_name, pretrained=False, num_classes=0)
            try:
                state_dict2 = torch.load(model_path, map_location='cpu', weights_only=True)
            except TypeError:
                state_dict2 = torch.load(model_path, map_location='cpu')
            self.backbone2.load_state_dict(state_dict2, strict=False)
        else:
            self.backbone2 = timm.create_model(backbone_name, pretrained=pretrained, num_classes=0)

        params = list(self.backbone2.parameters())
        freeze_until = int(len(params) * config["freeze_ratio"])
        for i, p in enumerate(params):
            p.requires_grad = i >= freeze_until     # freeze front part, train later part
        ...
        # 获取两个模型的特征维度
        self.feature_dim1 = self.backbone1.num_features
        self.feature_dim2 = self.backbone2.num_features
        print("Feature dim for backbone1:", self.feature_dim1)
        print("Feature dim for backbone2:", self.feature_dim2)

        # 拼接后的特征维度
        self.feature_dim = self.feature_dim1 + self.feature_dim2
        print("Concatenated feature dim:", self.feature_dim)
    ...
    def forward(self, img):
        # Extract features
        feat1 = self.backbone1(img)
        feat2 = self.backbone2(img)
            ...
            if should_save_cv:
                ...
                if len(top_cv_models) > 3:
                    ...
                    try:
                        ...
                        print(f"🗑️ 删除CV  模型: {os.path.basename(removed_cv[2])}")
                    except:
                        pass
...
def train_with_groupkfold(df_train, save_dir, config, device):
    ...
    if socket.gethostname() in ["user-PowerEdge-XE9680"]:
        tensor = allocate_gpu_memory(60, device)
...
config = {
    ...
    "backbone_name2"                 : "convnext_base.dinov3_lvd1689m",
