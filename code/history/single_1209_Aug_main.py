...
if socket.gethostname() == 'hao-2':
    ...
    DIRS = {
        "dir"  : dir,
        "train_extra1": Path(dir, "train_extra1"),
        "Aug_feature_Height": Path(dir, "历史模型/single_1209/single_1209_Classification_Height_Ave_cm/single_1209_Classification_Height_Ave_cm_swa_models"),
        "Aug_feature_NDVI"  : Path(dir, "历史模型/single_1209/single_1209_Classification_Pre_GSHH_NDVI/single_1209_Classification_Pre_GSHH_NDVI_swa_models"),
        "train": Path(dir, "train"),
        "test" : Path(dir, "test"),
        "model": Path(dir, "DualStream_multihead"),
        "data" : Path(dir),
...
class MySingleStreamModel(nn.Module):
    def __init__(self, 
        ...
        self.head_Pre_GSHH_NDVI  = make_head()
        self.head_Height_Ave_cm  = make_head()
    ...
    def forward(self, img):
        ...
        if config["task_name"] == "Pre_GSHH_NDVI":
            Pre_GSHH_NDVI = self.softplus(self.head_Pre_GSHH_NDVI(feat))
            preds = torch.cat([Pre_GSHH_NDVI], dim=1)  # [B, 1]
            return preds
        
        elif config["task_name"] == "Height_Ave_cm":
            Height_Ave_cm = self.softplus(self.head_Height_Ave_cm(feat))
            preds = torch.cat([Height_Ave_cm], dim=1)  # [B, 1]
            return preds
    ...
    def compute_loss(self, preds, targets):
        ...
        if config["task_name"] in ["Pre_GSHH_NDVI", "Height_Ave_cm"]:
    ...
    def __getitem__(self, idx):
        ...
        if config["task_name"] in ["Pre_GSHH_NDVI", "Height_Ave_cm"]:
            targets = torch.tensor(
                [float(row[config["task_name"]])],  # 关键：包一层 list
                dtype=torch.float32
            )
            aux_feats = None        
        elif config["task_name"] in ["main"]:
            targets = torch.tensor(
            ...
            aux_feats = torch.tensor(
                row[self.aux_cols].astype(float).values,
                dtype=torch.float32
            )
        return image, targets, aux_feats
...
def train_one_epoch(model, dataloader, optimizer, device, scaler, config):
    ...
    for step, (images, targets, aux_feats) in enumerate(dataloader):
        t_load = time.time()  
...
def get_fold_loaders(df, fold, config, device):
    ...
    # --- 打印数据统计信息 ---
    print(f"📊 Fold: {fold}")
    print(f"训练集样本数 (Train): {len(train_df)}")
    print(f"训练样本列名 (Train): {train_df.columns.tolist()}")
    print(f"验证集样本数 (Valid): {len(valid_df)}")
    print(f"验证样本列名 (Valid): {valid_df.columns.tolist()}")
...
def train_one_fold(fold, df, save_dir, config, device):
    ...
    for epoch in range(config["epochs"]):
        ...
        if should_stop(val_losses, threshold=config["cv_stability_stop_threshold"]):
...
def load_and_prepare_train_df():
    ...
    # df_file_path = Path(DIRS["dir"]) / "train_diffusion.csv"
    # df = pd.read_csv(df_file_path)
    ...
    # meta_cols = [
    #     "ID", "image_path", "Sampling_Date"
    # ]
    # df_meta = df[meta_cols].drop_duplicates(subset="ID")

    # df_train_part = pd.merge(df_meta, df_targets, on="ID", how="left")
    # show_df_info(df_train_part, "df_train_part")

    # df_final = df_train_part.copy()
...
def prepare_data_for_folds(df, config, save_dir):
    """
    为数据集添加折叠（fold）列，混合分组策略：
    - 对于有明确采集日期的样本，按日期分组以避免时空泄露。
    - 对于缺失采集日期的样本，将它们随机分配到各个折叠中。
    """
    print("🚀🚀🚀  数据分组")
    ...
    # 2. 处理有采集日期的样本和缺失采集日期的样本
    # 有采集日期的样本
    date_samples = df[df["Sampling_Date"] != "Unknown"].copy()
    # 缺失采集日期的样本
    unknown_samples = df[df["Sampling_Date"] == "Unknown"].copy()

    # 3. 对有采集日期的样本按采集日期生成分组 ID，并使用 GroupKFold
    unique_groups = np.random.permutation(date_samples["Sampling_Date"].unique())  # 随机排列采集日期
    group_map = {g: i for i, g in enumerate(unique_groups)}  # 为每个采集日期生成 ID
    date_samples["GroupID"] = date_samples["Sampling_Date"].map(group_map)

    # 4. 使用 GroupKFold 为有采集日期的样本分配 fold
    gkf = GroupKFold(n_splits=config["n_splits"])
    date_samples["fold"] = -1  # 初始化 fold 列
    for fold, (_, val_idx) in enumerate(gkf.split(date_samples, groups=date_samples["GroupID"])):
        date_samples.loc[val_idx, "fold"] = fold  # 给每个样本分配对应的 fold 编号




    # 5. 对缺失采集日期的样本，直接 -1
    unknown_samples["fold"] = -1 

    # 6. 合并有采集日期和缺失采集日期的样本
    df = pd.concat([date_samples, unknown_samples]).reset_index(drop=True)
    ...
    print(f"日期组数量: {len(unique_dates)}")

    # 统计缺失采集日期的样本数
    mask_no_date = df["Sampling_Date"] == "Unknown"
    print(f"独立随机样本数 (无日期): {mask_no_date.sum()}")

    # 最终总分组数 (Unique GroupIDs)
    print(f"最终总分组数 (Unique GroupIDs): {df['GroupID'].nunique()}")
    ...
    # 打印每个 fold 中有日期和无日期样本的分布
    print("\n各 Fold 中的日期与无日期样本数:")
    for fold in range(config["n_splits"]):
        fold_df = df[df["fold"] == fold]
        fold_with_date = fold_df[fold_df["Sampling_Date"] != "Unknown"]
        fold_without_date = fold_df[fold_df["Sampling_Date"] == "Unknown"]
        print(f"Fold {fold}: 日期样本数 = {len(fold_with_date)}, 无日期样本数 = {len(fold_without_date)}")

    # 打印 GroupID 分布
    group_counts = df["GroupID"].value_counts().sort_index()
    print("\n各 GroupID 样本分布:")
    print(group_counts)
    ...
    # --- 8. 保存和展示 ---
    output_path = Path(save_dir) / "data_for_folds.xlsx"
...
def train_with_groupkfold(df_train, save_dir, config, device):
    ...
    # df = prepare_data_for_folds(df, config, save_dir)
...
config = {
    ...
    # "task_name"                     : "Pre_GSHH_NDVI",
    # "task_name"                     : "Height_Ave_cm",
    "task_name"                     : "main",
    ...
    "cv_stability_stop_threshold"    : 5e-8,
    "min_epoch"                      : 0,
    ...
    "use_lora"                       : False,    # 开启 LoRA 微调
    "lora_r"                         : 4,       # 秩大小。样本少，建议 4 或 8；显存多且效果一般可升至 16
...
if (
    ...
    if DEBUG:
        # 2. 关键：reset_index(drop=True) 会丢弃旧索引，生成从 0 开始的新索引
        df_train = df_train.sample(n=50, random_state=42).reset_index(drop=True).copy()
...
def generate_test_aug_features(config):
...
if __name__ == "__main__" and config["task_name"] == "main" and isPREDICT:
    print("\n🧠 Starting prediction pipeline...")
    if isTRAIN and socket.gethostname() in ["user-PowerEdge-XE9680", "hao-2","simon-MS-7D94"]:
        print(idx,device)
        pass
    else:
...
print("completed_completed_completed_completed")