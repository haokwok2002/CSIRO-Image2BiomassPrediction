...
def freeze_model(model, freeze_ratio=1.0):
    params = list(model.backbone.parameters())
    freeze_until = int(len(params) * freeze_ratio)
    for i, p in enumerate(params):
        p.requires_grad = i >= freeze_until     # freeze front part, train later part
    print(f"🔒 冻结模型参数比例: {freeze_ratio:.2f} ({freeze_until}/{len(params)})")
...
def train_one_fold(fold, df, save_dir, config, device):
    ...
    freeze_model(model, freeze_ratio=1.0)  # 完全冻结前面的层
    for epoch in range(config["epochs"]):
        epoch_start = time.time()

        if epoch == config["change_freeze_ratio2state1"]:  # 当 epoch 达到20时，可以根据配置调整冻结比例
            freeze_model(model, freeze_ratio=config["freeze_ratio_state1"])  # 读取配置的冻结比例
...
config = {
    ...
    "change_freeze_ratio2state1"     : 30,  # 在第20个epoch后调整冻结比例
    "freeze_ratio_state1"            : 0.8,  # 调整后的冻结比例
    ...
    "backbone_name"                  : "vit_huge_plus_patch16_dinov3.lvd1689m", #"vit_base_patch16_dinov3.lvd1689m",
...
if (
    ...
    if DEBUG:
        df_train = df_train.iloc[:50].copy()
